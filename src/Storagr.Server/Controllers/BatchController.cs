...
namespace Storagr.Server.Controllers
{
    ...
    public class BatchController : StoragrController
    {
        ...
        private async Task<StoragrBatchResponse> Download(RepositoryEntity repository, StoragrBatchRequest batchRequest, CancellationToken cancellationToken)
        {
            ...
            var requestedObjects = batchRequest.Objects.ToList();
            var requestedIds = requestedObjects.Select(v => v.ObjectId);
            var objects = (await _objectService.GetMany(repository.Id, requestedIds, cancellationToken)).ToList();

            var responseObjectsAsync = requestedObjects.Select(async v =>
            {
                if (!objects.Exists(x => x.Id == v.ObjectId && x.Size == v.Size))
                {
